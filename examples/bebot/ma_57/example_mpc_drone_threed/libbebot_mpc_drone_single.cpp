...
public:
    PointSetProblem(int N, double tf, double delta_v_max, double delta_v_min, 
        ...
        std::cout << "[DBG] N_=" << N_ << " (N_+1)=" << (N_+1) << " tf_=" << tf_ << "\n";
    ...
    virtual bool get_nlp_info(Index& n, Index& m, Index& nnz_jac_g, Index& nnz_h_lag, IndexStyleEnum& index_style) {
        ...
        std::cout << "[DBG] get_nlp_info: N_=" << N_
          << " n=" << (10*(N_+1))
          << " m=" << (10*(N_+1)) << "\n";
    ...
    //     const Number x0[60] = {
    //         -19.9700, -19.9500, -19.9404, -19.9884, -20.2855, -20.0000,
    //         0.0000, -0.0000,  0.0003,  0.0026,  0.0242,  0.0017,
    //         0.0200,  0.0075, -0.0103, -0.0365, -0.1280, -0.0085,
    //         0.0000,  0.0001,  0.0005,  0.0016,  0.0005, -0.0000,
    //         0.0000,  0.0000,  0.0002,  0.0003,  0.0000, -0.0000,
    //         -0.0000,  0.0001,  0.0000, -0.0000, -0.0000, -0.0000,
    //         0.1300,  0.1502,  0.1281, -0.1292, -0.7815, -1.4387,
    //         -2.8600,  1.2833, -6.0117, -0.2903, -1.7247, -3.1806,
    //         4.0200,  4.6200,  5.6416,  5.2993,  0.8453,  7.4166,
    //         0.4200, -0.0156, -0.0559, -0.0244, -0.0040,  0.0000
    //     };

    //     for (Index i = 0; i < n; ++i) x[i] = x0[i];  // expects n==60
    //     return true;
    // }
    ...
    virtual bool eval_f(Index n, const Number* x, bool new_x, Number& obj_value) {
        ...
        // ---- print dv/dm/ds/dh vectors (paste right after you create them) ----
        std::cout << std::setprecision(16);

        std::cout << "dv_vector = [";
        for (int i = 0; i < N_ + 1; ++i) {
            std::cout << dv_vector[i] << (i + 1 < N_ + 1 ? ", " : "");
        }
        std::cout << "]\n";

        std::cout << "dm_vector = [";
        for (int i = 0; i < N_ + 1; ++i) {
            std::cout << dm_vector[i] << (i + 1 < N_ + 1 ? ", " : "");
        }
        std::cout << "]\n";

        std::cout << "ds_vector = [";
        for (int i = 0; i < N_ + 1; ++i) {
            std::cout << ds_vector[i] << (i + 1 < N_ + 1 ? ", " : "");
        }
        std::cout << "]\n";

        std::cout << "dh_vector = [";
        for (int i = 0; i < N_ + 1; ++i) {
            std::cout << dh_vector[i] << (i + 1 < N_ + 1 ? ", " : "");
        }
        std::cout << "]\n";
        ...
        // ---- print state_term and control_term (drop this right after you compute them) ----
        // std::cout << std::setprecision(16);

        // std::cout << "sum_z_state="     << sum_z_state
        //         << "  sum_theta_state=" << sum_theta_state
        //         << "  sum_psi_state="   << sum_psi_state << "\n";

        // std::cout << "state_term=" << state_term << "\n";
        ...
        // std::cout << "control_term=" << control_term << "\n";

        // std::cout << "obj_value (state+control)=" << (state_term + control_term) << "\n";
        // ---------------------------------------------------------------
    ...
    virtual bool eval_g(Index n, const Number* x, bool new_x, Index m, Number* g) {
        ...
        // ---- quick debug prints (paste right after the vector unpack) ----
        ...
        // X2_cd(:,k) = C*Xh(:,k) + D*Uh(:,k)  for k = 0..N
        // Xh has 2 rows: [psi; r]
        // Uh has 1 row: [dh]

        // for (int k = 0; k < (N_ + 1); ++k) {
        //     const double psi_k = X1_matrix_flat_cd[0 * (N_ + 1) + k];
        //     const double r_k   = X1_matrix_flat_cd[1 * (N_ + 1) + k];

        //     const double dh_k  = U_matrix_flat_cd[k];

        //     // Row 0
        //     // X2_matrix_flat_cd[0 * (N_ + 1) + k] =
        //     //     C_[0][0] * psi_k + C_[0][1] * r_k +
        //     //     D_[0][0] * dh_k;

        //     // Row 1
        //     X2_matrix_flat_cd[1 * (N_ + 1) + k] =
        //         C_[1][0] * psi_k + C_[1][1] * r_k +
        //         D_[1][0] * dh_k;
        // }
...
extern "C" {
    ...
    void solve_point_set_problem(PointSetProblem* problem) {
        ...
        // // optional but often helpful for OCP/MPC:
        // app->Options()->SetNumericValue("nlp_scaling_max_gradient", 1e4);   // default-ish, try 100..1000
        // app->Options()->SetNumericValue("nlp_scaling_min_value", 1e-8);   

        // app->Options()->SetStringValue("scaled_termination", "yes");

        // // accept a “good enough” solution sooner:
        // // Acceptable (early stop) criteria
        // app->Options()->SetNumericValue("acceptable_tol", 1e-2);
        // app->Options()->SetNumericValue("acceptable_constr_viol_tol", 1e-4); // or 1e-5
        // app->Options()->SetNumericValue("acceptable_dual_inf_tol", 1e-1);
        // app->Options()->SetIntegerValue("acceptable_iter", 1);  // stop as soon as acceptable is met
