...
public:
    ...
    bool get_starting_point(
        ...
        // ----------------------------------------------------
        // Optional externally supplied primal initial guess.
        // Used for NN-informed initialization / benchmarking.
        // ----------------------------------------------------
        if (
            custom_initial_guess_active_
            && custom_initial_guess_.size()
               == static_cast<size_t>(n)
        ) {
            std::copy(
                custom_initial_guess_.begin(),
                custom_initial_guess_.end(),
                x
            );
    ...
    // ========================================================
    // External initial-guess support
    // ========================================================
    void set_constant_initial_guess(
        double value
    ) {
        custom_initial_guess_.assign(
            12 * L_,
            value
        );
    ...
    bool set_position_yaw_initial_guess(
        const double* position_yaw_guess,
        int n_values
    ) {
        if (
            !position_yaw_guess
        ...
        // ----------------------------------------------------
        // 1) Copy NN-predicted Px, Py, Pz, Psi coefficients.
        // ----------------------------------------------------
        std::copy(
            position_yaw_guess,
            position_yaw_guess + 4 * L_,
            custom_initial_guess_.begin()
        );

        // ----------------------------------------------------
        // 2) Enforce known initial position/yaw exactly.
        // ----------------------------------------------------
        custom_initial_guess_[0 * L_] =
            px_cur_;

        custom_initial_guess_[1 * L_] =
            py_cur_;

        custom_initial_guess_[2 * L_] =
            pz_cur_;

        custom_initial_guess_[3 * L_] =
            psi_cur_;

        // For the canonical training/benchmark problem the
        // initial translational velocity and yaw rate are zero.
        // Setting the second position/yaw coefficient equal to
        // the first gives the NN guess the same known initial
        // derivative structure observed in the OCP solutions.
        if (L_ > 1) {
            if (std::abs(vx_cur_) < 1e-12) {
                custom_initial_guess_[0 * L_ + 1] =
                    px_cur_;
            }

            if (std::abs(vy_cur_) < 1e-12) {
                custom_initial_guess_[1 * L_ + 1] =
                    py_cur_;
            }

            if (std::abs(vz_cur_) < 1e-12) {
                custom_initial_guess_[2 * L_ + 1] =
                    pz_cur_;
            }

            if (std::abs(w_cur_) < 1e-12) {
                custom_initial_guess_[3 * L_ + 1] =
                    psi_cur_;
            }
        }
        ...
        // ----------------------------------------------------
        // 3) Construct Vx, Vy, Vz, W from the predicted
        //    position/yaw curves using the SAME discrete
        //    dynamics relation used in the NLP:
        //
        //       Dm^T q - r_tau * v = 0
        //
        // therefore:
        //
        //       v_i = (Dm^T q)_i / r_tau_i.
        // ----------------------------------------------------
        for (int block = 0; block < 4; ++block) {
            const int q_offset =
                block * L_;
        ...
        // Enforce known initial velocity/yaw-rate values exactly.
        custom_initial_guess_[4 * L_] =
            vx_cur_;

        custom_initial_guess_[5 * L_] =
            vy_cur_;

        custom_initial_guess_[6 * L_] =
            vz_cur_;

        custom_initial_guess_[7 * L_] =
            w_cur_;

        // ----------------------------------------------------
        // 4) Construct Ax, Ay, Az, Aw from Vx, Vy, Vz, W:
        //
        //       Dm^T v - r_tau * a = 0
        //
        // therefore:
        //
        //       a_i = (Dm^T v)_i / r_tau_i.
        // ----------------------------------------------------
        for (int block = 4; block < 8; ++block) {
            const int v_offset =
                block * L_;
    ...
    void clear_external_initial_guess() {
        custom_initial_guess_.clear();
...
private:
    ...
    // Optional externally supplied primal initialization.
    bool custom_initial_guess_active_ = false;
    std::vector<Number> custom_initial_guess_;
...
static int solve_point_set_problem_impl(
    PointSetProblem* problem,
    int print_level
) {
    if (!problem) {
...
int solve_point_set_problem(
    ...
    return solve_point_set_problem_impl(
        problem,
        3
    );
}

int solve_point_set_problem_quiet(
    PointSetProblem* problem
) {
    return solve_point_set_problem_impl(
        problem,
...
int set_constant_initial_guess(
    PointSetProblem* problem,
    double value
) {
    if (!problem) {
    ...
    problem->set_constant_initial_guess(
        value
    );
...
int set_position_yaw_initial_guess(
    PointSetProblem* problem,
    const double* position_yaw_guess,
    int n_values
) {
    ...
    return problem->set_position_yaw_initial_guess(
        position_yaw_guess,
        n_values
    )
        ? 1
...
void clear_external_initial_guess(
    ...
    if (problem) {
        problem->clear_external_initial_guess();
    }
}
