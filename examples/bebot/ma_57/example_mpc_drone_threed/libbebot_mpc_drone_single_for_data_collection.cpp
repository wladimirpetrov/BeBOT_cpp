...
//         const double w_p = 0.01; // 1
//         const double w_psi = 1; // 1
//         const double w_a = 0.01; // 0.01
//         const double w_aw = 0.001; // 0.001
...
public:
    ...
    virtual bool get_nlp_info(
        ...
        /*
            ...
            New constraints:
            4 * N: monotonicity constraints for px, py, pz, psi
    ...
    virtual bool get_bounds_info(
        ...
        /*
            ...
            Each control point is forced to stay between current value and target value.

            Example:
            px_cur = 2.0, pxf = 2.002

            Then all px control points satisfy:
            2.0 <= px_i <= 2.002

            If:
            px_cur = 2.0, pxf = 1.998

            Then all px control points satisfy:
            1.998 <= px_i <= 2.0

            The first control point is still pinned to the current measured state.
        */
        auto apply_current_to_target_bounds = [&](int block,
                                                  double q_cur,
                                                  double q_target,
                                                  double q_min,
                                                  double q_max) {
            const int off = block * L;

            const double raw_lo = std::min(q_cur, q_target);
            const double raw_hi = std::max(q_cur, q_target);

            const double lo = std::max(q_min, raw_lo);
            const double hi = std::min(q_max, raw_hi);
        ...
        // px
        apply_current_to_target_bounds(
            0,
            px_cur_,
            pxf_,
            px_min_,
        ...
        /*
            ...
            Existing:
            g[0 ... 8*(N+1)-1]      = dynamics equalities
            g[8*(N+1) ... 9*(N+1)-1]  = speed inequality <= 0
            g[9*(N+1) ... 10*(N+1)-1] = acceleration inequality <= 0

            New:
            g[10*(N+1) ... 10*(N+1)+4*N-1] = monotonic constraints >= 0
        */
        ...
        // Speed constraint: c_speed <= 0
        for (int i = 8 * L; i < 9 * L; ++i) {
            g_l[i] = -std::numeric_limits<double>::infinity();
            g_u[i] = 0.0;
        }

        // Acceleration constraint: c_accel <= 0
        for (int i = 9 * L; i < 10 * L; ++i) {
            g_l[i] = -std::numeric_limits<double>::infinity();
            g_u[i] = 0.0;
        }

        // Monotonicity constraints: monotonic_value >= 0
        const int mono_start = 10 * L;

        for (int i = mono_start; i < mono_start + 4 * N_; ++i) {
            g_l[i] = 0.0;
    ...
    virtual bool get_starting_point(
        ...
        auto fill_linear = [&](int block, double q0, double qf) {
            const int off = block * L;
            ...
            for (int i = 0; i < L; ++i) {
                const double alpha =
                    (N_ == 0)
                        ? 0.0
                        : static_cast<double>(i) / static_cast<double>(N_);

                x[off + i] = q0 + alpha * (qf - q0);
            }
        };

        auto fill_constant = [&](int block, double value) {
            const int off = block * L;
        ...
        // Monotonic initial guess for planned outputs
        fill_linear(0, px_cur_,  pxf_);
        fill_linear(1, py_cur_,  pyf_);
        fill_linear(2, pz_cur_,  pzf_);
        fill_linear(3, psi_cur_, psif_);

        // Velocity initial guesses
        fill_constant(4, vx_cur_);
        fill_constant(5, vy_cur_);
        fill_constant(6, vz_cur_);
        fill_constant(7, w_cur_);

        // Acceleration initial guesses
        fill_constant(8,  0.0);
        fill_constant(9,  0.0);
        fill_constant(10, 0.0);
        fill_constant(11, 0.0);
    ...
    virtual bool eval_f(
        ...
        vdMul(L, r_tau.data(), px_diff_sqr.data(),  px_weighted.data());
        vdMul(L, r_tau.data(), py_diff_sqr.data(),  py_weighted.data());
        vdMul(L, r_tau.data(), pz_diff_sqr.data(),  pz_weighted.data());
        vdMul(L, r_tau.data(), psi_diff_sqr.data(), psi_weighted.data());
    ...
    virtual bool eval_g(
        ...
        static int g_call = 0;
        const bool do_print = (g_call < 1000);
        ++g_call;
        ...
        std::vector<double> tau_vec(L);
        std::vector<double> r_tau_row(L);
        ...
        vdMul(L, vx_vector.data(), r_tau_row.data(), px_rhs_scaled.data());
        vdMul(L, vy_vector.data(), r_tau_row.data(), py_rhs_scaled.data());
        vdMul(L, vz_vector.data(), r_tau_row.data(), pz_rhs_scaled.data());
        vdMul(L, w_vector.data(),  r_tau_row.data(), psi_rhs_scaled.data());

        vdMul(L, ax_vector.data(), r_tau_row.data(), vx_rhs_scaled.data());
        vdMul(L, ay_vector.data(), r_tau_row.data(), vy_rhs_scaled.data());
        vdMul(L, az_vector.data(), r_tau_row.data(), vz_rhs_scaled.data());
        vdMul(L, aw_vector.data(), r_tau_row.data(), w_rhs_scaled.data());
        ...
        /*
            New monotonicity constraints.

            For each variable q in px, py, pz, psi:
            ...
            If target < current:
                -(q[i+1] - q[i]) >= 0
                which means q[i+1] <= q[i]

            Combined:
                direction * (q[i+1] - q[i]) >= 0

            where direction = +1 if target >= current, otherwise -1.
        */
        const int mono_start = 10 * L;

        const double dir_px  = (pxf_  >= px_cur_)  ? 1.0 : -1.0;
        const double dir_py  = (pyf_  >= py_cur_)  ? 1.0 : -1.0;
        const double dir_pz  = (pzf_  >= pz_cur_)  ? 1.0 : -1.0;
        const double dir_psi = (psif_ >= psi_cur_) ? 1.0 : -1.0;

        for (Index i = 0; i < N_; ++i) {
            g[mono_start + 0 * N_ + i] =
                dir_px * (px_vector[i + 1] - px_vector[i]);

            g[mono_start + 1 * N_ + i] =
                dir_py * (py_vector[i + 1] - py_vector[i]);

            g[mono_start + 2 * N_ + i] =
                dir_pz * (pz_vector[i + 1] - pz_vector[i]);

            g[mono_start + 3 * N_ + i] =
                dir_psi * (psi_vector[i + 1] - psi_vector[i]);
        }
    ...
    virtual void finalize_solution(
        ...
        auto px_norm =
            BernsteinPoly(px_2d, t_norm, 0.0, 1.0);

        auto py_norm =
            BernsteinPoly(py_2d, t_norm, 0.0, 1.0);

        auto pz_norm =
            BernsteinPoly(pz_2d, t_norm, 0.0, 1.0);

        auto psi_norm =
            BernsteinPoly(psi_2d, t_norm, 0.0, 1.0);

        auto vx_norm =
            BernsteinPoly(vx_2d, t_norm, 0.0, 1.0);

        auto vy_norm =
            BernsteinPoly(vy_2d, t_norm, 0.0, 1.0);

        auto vz_norm =
            BernsteinPoly(vz_2d, t_norm, 0.0, 1.0);

        auto w_norm =
            BernsteinPoly(w_2d, t_norm, 0.0, 1.0);

        auto ax_norm =
            BernsteinPoly(ax_2d, t_norm, 0.0, 1.0);

        auto ay_norm =
            BernsteinPoly(ay_2d, t_norm, 0.0, 1.0);

        auto az_norm =
            BernsteinPoly(az_2d, t_norm, 0.0, 1.0);

        auto aw_norm =
            BernsteinPoly(aw_2d, t_norm, 0.0, 1.0);
